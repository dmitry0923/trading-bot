...
class PortfolioRiskEngineClusterTest {
    ...
    private fun stubVols(tickers: List<String>, volPercent: Double) {
        tickers.forEach { t ->
    ...
    @Test
    fun `three correlated longs are one market bet and get blocked`() = runBlocking {
    ...
    @Test
    fun `perfectly correlated cluster has effective positions of one`() = runBlocking {
    ...
    @Test
    fun `independent equal positions are not a cluster and allowed`() = runBlocking {
        stubVols(listOf("A", "B", "C"), 1.0)
    ...
    @Test
    fun `single position is baseline and not blocked by concentration`() = runBlocking {
    ...
    @Test
    fun `moderate var scales down position size instead of blocking`() = runBlocking {
    ...
    @Test
    fun `blocked mode off only scales and never rejects`() = runBlocking {
    ...
    @Test
    fun `disabled engine allows everything`() = runBlocking {
