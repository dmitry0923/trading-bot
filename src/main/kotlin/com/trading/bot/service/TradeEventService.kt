...
class TradeEventService(
    ...
    private suspend fun append(pos: Position, type: String, payload: Map<String, Any?> = snapshot(pos)) {
        try {
    ...
    private fun snapshot(pos: Position): Map<String, Any?> = mapOf(
     ...
     */
    private fun aggregateIdOf(positionId: Long?): UUID =
        UUID.nameUUIDFromBytes("position:${positionId ?: -1L}".toByteArray())
}
