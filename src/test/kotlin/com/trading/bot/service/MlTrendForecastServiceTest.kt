...
class MlTrendForecastServiceTest {
    ...
    @Test
    fun `forecast ranks tickers by trend score and respects topN`() {
        config.enabled = true
        ...
        val result = runBlocking { service.forecast(listOf("SBER", "GAZP"), topN = 1) }

        assertEquals("OK", result.mode)
        assertEquals(1, result.topN)
        assertEquals(config.trend.horizonBars, result.horizonBars)
        assertEquals(1, result.candidates.size)
        // SBER (растущий тренд) — LONG с сильными индикаторами, GAZP (нисходящий) — SHORT.
        ...
        // 0.6 * 0.85 + 0.4 * 1.0 (все индикаторы за LONG) = 0.91.
        assertEquals(0.91, result.candidates[0].trendScore)
        // Каждый тикер прогнался в обоих направлениях.
        assertEquals(4, model.calls.size)
    }

    @Test
    fun `forecast returns both candidates when topN exceeds count`() {
        config.enabled = true
        val now = LocalDateTime.now()
        val model = RecordingModel(longRsiProb = 0.85, shortRsiProb = 0.2, lowRsiLong = 0.15, lowRsiShort = 0.6)
        runBlocking {
        ...
        val result = runBlocking { service.forecast(listOf("SBER", "GAZP"), topN = 5) }

        assertEquals(2, result.candidates.size)
        assertTrue(result.candidates[0].trendScore >= result.candidates[1].trendScore)
        assertEquals("SHORT", result.candidates[1].direction)
        assertEquals(0.76, result.candidates[1].trendScore)
    }

    @Test
    fun `forecast skips tickers with insufficient candle history`() {
        config.enabled = true
        val now = LocalDateTime.now()
        runBlocking {
        ...
        val result = runBlocking { service.forecast(listOf("SBER", "GAZP"), topN = null) }
    ...
    @Test
    fun `forecast uses latest macro snapshot at or before now and sets blind spot flag`() {
        config.enabled = true
        val now = LocalDateTime.now()
        ...
        val result = runBlocking { service.forecast(listOf("SBER"), topN = null) }

        assertEquals(1, result.candidates.size)
        assertEquals(1, result.candidates[0].inBlindSpotHour)
        assertEquals(now.hour, result.candidates[0].hourOfDay)
        // cbr_rate на позиции 9 числовых признаков — из снапшота, а не из фолбэка.
    ...
    @Test
    fun `forecast returns 503 when model unavailable`() {
        config.enabled = true
        runBlocking {
        ...
        val ex =
            assertThrows<ResponseStatusException> {
                runBlocking { service.forecast(listOf("SBER"), topN = null) }
            }
    ...
    @Test
    fun `forecast returns 400 when no tickers`() {
        config.enabled = true
        runBlocking {
            Mockito.`when`(modelProvider.model).thenReturn(RecordingModel(0.7, 0.3, 0.4, 0.6))
        }
        ...
        val ex =
            assertThrows<ResponseStatusException> {
                runBlocking { service.forecast(emptyList(), topN = null) }
            }
    ...
    @Test
    fun `forecast uses config topN by default`() {
        config.enabled = true
        config.trend.topN = 1
        val now = LocalDateTime.now()
        runBlocking {
