 ...
 * min(1 000 ₽, 5 000 ₽) = 1 000 ₽ при AUM 50k (DrawdownProtectionService
 * effectiveDailyLossLimitRub); «5 000 ₽» из maxDailyLossRub — лишь верхний
 * абсолютный потолок. Указание «10% = 5 000 ₽» в ранних доках — УСТАРЕЛО
 * (лимит понижен до 2%).
...
class RiskConfig {
    ...
    /**
     * Максимально допустимый возраст фьючерсных risk-инпутов (ГО, свободные средства),
     * мс. Если кэшированное значение старше [maxGoAgeMs] — [AlorFuturesClient]
     * перезапрашивает API; при недоступности API возвращает null (fail-closed):
     * сайзинг по УСТАРЕВШЕМУ ГО/балансу запрещён (P0-аудит, паритет EXEC-005).
     * 30000 = 30 секунд — данные старше 30 с не участвуют в принятии решения.
     */
    var maxGoAgeMs: Long = 30_000

    /**
     * Множитель стресс-нагрузки маржинального гейта (P1-аудит): требуемое ГО кандидата
     * в [RiskManagementService.exceedsMarginUtilization] умножается на этот фактор —
     * гейт оценивает залоговую загрузку с запасом (1.5 = запас 50% на
     * рост/пересчёт ГО после открытия). Защита от входа на пределе лимита, когда
     * малейшее повышение ГО относительно текущего расчёта приведёт к margin call.
     */
    var stressedMarginMultiplier: Double = 1.5
