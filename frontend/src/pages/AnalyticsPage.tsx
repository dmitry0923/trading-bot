  ...
  return (
    <div>
      <Section title="Health">
        {healthError && <div style={{ color: '#c62828' }}>Error: {healthError}</div>}
        {health && <pre style={{ background: '#f5f5f5', padding: 12 }}>{JSON.stringify(health, null, 2)}</pre>}
      </Section>
        ...
        <table border={1} cellPadding={6} style={{ borderCollapse: 'collapse', width: '100%' }}>
          <thead style={{ background: '#f0f0f0' }}>
            <tr>
              ...
              <th>SL%</th><th>TP%</th><th>Best Hour</th><th>Worst Hour</th>
            </tr>
          </thead>
          <tbody>
      ...
      <Section title="Blind Spots">
        {(blindSpots || []).map((b, i) => (
          <div key={b.id || i} style={{ padding: 6, borderBottom: '1px solid #eee' }}>
            {b.conditionPattern} <span style={{ color: '#888' }}>({b.description || ''})</span>
          </div>
        ))}
        {(!blindSpots || blindSpots.length === 0) && <div style={{ color: '#888' }}>No active blind spots</div>}
      </Section>
      ...
      <Section title={`Time Pattern (${ticker})`}>
        ...
        {timePattern && <pre style={{ background: '#f5f5f5', padding: 12 }}>{JSON.stringify(timePattern, null, 2)}</pre>}
      </Section>

      <Section title={`Adaptive Params (${ticker})`}>
        {adaptive && <pre style={{ background: '#f5f5f5', padding: 12 }}>{JSON.stringify(adaptive, null, 2)}</pre>}
      </Section>
    </div>
  );
}
