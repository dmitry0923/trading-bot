...
const AGENT_OPTIONS = [
  ...
  'Agent-4-Contrarian', 'Agent-5-Arbitrator', 'Agent-6-Performance'
];
  ...
  return (
    <div>
      <div style={{ marginBottom: 12, display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
        <input placeholder="Ticker" value={ticker} onChange={e => setTicker(e.target.value)} style={{ padding: 6 }} />
        <select value={agent} onChange={e => setAgent(e.target.value)} style={{ padding: 6 }}>
          {AGENT_OPTIONS.map(a => (
      ...
      <table border={1} cellPadding={6} style={{ borderCollapse: 'collapse', width: '100%' }}>
        <thead style={{ background: '#f0f0f0' }}>
          <tr>
            ...
            <th>Latency</th><th>Tokens</th><th>Cached</th><th>Reasoning</th>
          </tr>
        </thead>
        <tbody>
          {(logs || []).map(l => (
