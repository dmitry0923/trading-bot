...
class CnyRubStrategyTest {
    ...
    @Test
    fun `returns HOLD for non CNYRUB_TOM ticker`() =
        runBlocking {
            val ctx =
                context(
                    "SBER",
                    indicators =
                        indicators(
                            rsi = 25.0,
                            bbLower = BigDecimal("99"),
                            bbUpper = BigDecimal("101"),
    ...
    @Test
    fun `returns HOLD without indicators`() =
        runBlocking {
    ...
    @Test
    fun `BUY when RSI oversold and price at lower BB`() =
        runBlocking {
            val ctx =
                context(
                    "CNYRUB_TOM",
    ...
    @Test
    fun `SELL when RSI overbought and price at upper BB`() =
        runBlocking {
            val ctx =
                context(
                    "CNYRUB_TOM",
    ...
    @Test
    fun `HOLD when RSI oversold but price not at lower BB`() =
        runBlocking {
            val ctx =
                context(
                    "CNYRUB_TOM",
    ...
    @Test
    fun `microstructure confirms buy`() =
        runBlocking {
            val ctx =
                context(
                    "CNYRUB_TOM",
    ...
    @Test
    fun `microstructure blocks buy when obi opposes`() =
        runBlocking {
            val ctx =
                context(
                    "CNYRUB_TOM",
    ...
    @Test
    fun `fallback mode works without bidSize askSize`() =
        runBlocking {
            val ctx =
                context(
                    "CNYRUB_TOM",
    ...
    @Test
    fun `signal strength above zero for valid signal`() =
        runBlocking {
            val ctx =
                context(
                    "CNYRUB_TOM",
    ...
    private fun context(
        ...
        val microprice =
            if (bidSize != null && askSize != null) {
                com.trading.bot.domain.microstructure.MicropriceCalculator
                    .calculate(bid, ask, bidSize, askSize)
            } else {
                null
            }
