...
class StockEntryProfileTest {
    ...
    private val profile = StockEntryProfile(
    ...
    @Test
    fun `sizePosition CNYRUB_TOM Kelly 0_25 of 50k gives 1 lot`() = runBlocking {
        Mockito.doReturn(BigDecimal("50000")).`when`(aumProvider).currentAum(null)
        Mockito.doReturn(BigDecimal("12500")).`when`(adaptiveRisk)
    ...
    @Test
    fun `sizePosition returns KELLY_BELOW_MIN_LOT when MinimumLotPolicy disabled`() = runBlocking {
    ...
    @Test
    fun `sizePosition returns KELLY_BELOW_MIN_LOT when Kelly fraction below threshold`() = runBlocking {
        ...
        riskConfig.minimumLotPolicyEnabled = true
        Mockito.doReturn(BigDecimal("50000")).`when`(aumProvider).currentAum(null)
        Mockito.doReturn(BigDecimal("3000")).`when`(adaptiveRisk)
            .calculateOptimalPositionSize("CNYRUB_TOM", signalStrength = 0.7)
    ...
    @Test
    fun `sizePosition normal Kelly when Kelly equals notionalPerLot`() = runBlocking {
        // kellySizeRub = 12420 = notionalPerLot
        // kellyLots = floor(12420 / 12420) = 1 → normal Kelly path, not override
        Mockito.doReturn(BigDecimal("50000")).`when`(aumProvider).currentAum(null)
        Mockito.doReturn(BigDecimal("12420")).`when`(adaptiveRisk)
            .calculateOptimalPositionSize("CNYRUB_TOM", signalStrength = 0.7)
    ...
    @Test
    fun `sizePosition normal Kelly when Kelly just above notional`() = runBlocking {
        // kellySizeRub = 12420.01, notionalPerLot = 12420
        // kellyLots = floor(12420.01/12420) = 1 → normal Kelly path
        Mockito.doReturn(BigDecimal("50000")).`when`(aumProvider).currentAum(null)
        Mockito.doReturn(BigDecimal("12420.01")).`when`(adaptiveRisk)
            .calculateOptimalPositionSize("CNYRUB_TOM", signalStrength = 0.7)
    ...
    @Test
    fun `sizePosition returns ZERO_RISK_SIZE when Kelly rejects trade`() = runBlocking {
        // kellySizeRub = 0 → Kelly has negative expected value → no override possible
        Mockito.doReturn(BigDecimal("50000")).`when`(aumProvider).currentAum(null)
        Mockito.doReturn(BigDecimal.ZERO).`when`(adaptiveRisk)
            .calculateOptimalPositionSize("CNYRUB_TOM", signalStrength = 0.7)
    ...
    @Test
    fun `sizePosition returns KELLY_BELOW_MIN_LOT when risk cap blocks 1 lot`() = runBlocking {
    ...
    @Test
    fun `sizePosition lossPerLot includes 2x commission`() = runBlocking {
    ...
    @Test
    fun `postSizingChecks returns ZERO_RISK_SIZE when quantity is 0`() = runBlocking {
        val size = PositionSizeResult(0, BigDecimal.ZERO, BigDecimal.ZERO, null, null)
        val result = profile.postSizingChecks(
            "CNYRUB_TOM", PositionDirection.LONG, BigDecimal("12.42"), size, emptyList()
        )
        assertEquals("ZERO_RISK_SIZE", result)
    }

    @Test
    fun `postSizingChecks returns null when portfolio within limits`() = runBlocking {
    ...
    @Test
    fun `postSizingChecks returns PORTFOLIO_LIMIT when exposure exceeded`() = runBlocking {
        val size = PositionSizeResult(1, BigDecimal("12420"), BigDecimal("500"), null, null)
    ...
    fun `buildPosition sets instrumentType from spec`() {
        val decision = TradeRiskDecision(
            ticker = "CNYRUB_TOM",
            cycleId = "cycle-1",
            timeframe = "MINUTE_10",
            action = StrategyAction.BUY,
    ...
    fun `buildOrderParams delegates to OrderBuilder buildSpotOrderParams`() {
        ...
        val mockParams = com.trading.bot.domain.order.OrderParams(
            direction = PositionDirection.LONG,
            quantity = 1,
        )
    ...
    @Test
    fun `preSizingChecks returns null when no correlation limits exceeded`() = runBlocking {
    ...
    @Test
    fun `preSizingChecks returns CORRELATION when global correlation exceeded`() = runBlocking {
    ...
    @Test
    fun `preSizingChecks returns SECTOR_CORRELATION when sector correlation exceeded`() = runBlocking {
        whenever(adaptiveRisk.exceedsCorrelationLimit(any(), any())).thenReturn(false)
