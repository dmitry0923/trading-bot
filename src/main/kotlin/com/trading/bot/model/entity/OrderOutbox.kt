...
enum class OutboxStatus {
    ...
    /**
     * Терминальное состояние: ордер долетел до биржи, но получил определённый
     * отказ ([OrderRejectedException], REST 4xx кроме 429 / WS rejected-событие) —
     * никогда не переотправляется worker'ом. В отличие от [FAILED] (доставка не
     * подтверждена / не отправлено — безопасно ретраить), [REJECTED] — ответ биржи
     * «нет», повторная отправка приведёт к тому же исходу.
     */
    REJECTED,
}
