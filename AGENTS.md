...
### ВНИМАНИЕ (13.34): цифры фьючерсов переоценены — добавилась симуляция ликвидации

`bt.futures-liquidation-simulation=true` (по умолчанию) с 2026-08-31: фьючерсная
...
паритет live-monitorу. Прошлые фьючерсные калибровки (RI/CNYRUBF ниже) рассчитаны
БЕЗ ликвидации и **завышены** (overestimated) — позиция могла удерживаться после
прохождения liq-уровня и отыгрываться.

Следствия:
- Все цифры фьючерсов в AGENTS.md (этот раздел, CNYRUBF 90%, RI re-калибровка)
  требуют пересчёта с ликвидацией перед любым live-запуском.
- Ожидаемое направление: доходность ниже / MDD выше, т.к. liq-филл хуже стопного.
- Для отключения реалистичной ликвидации в stress/сравнительном прогоне —
  `bt.futures-liquidation-simulation=false`.
