...
Захардкоженные константы текущей версии:

| Константа | Значение | Комментарий |
|---|---|---|
| Слайс капитала | `20%` | соответствует духу `max-position-rub` в живом боте |
| Стоп-лосс | `2%` | соответствует `risk.default-stop-loss-percent` |
| Тейк-профит | `4%` | соответствует `risk.default-take-profit-percent` |
| Min лот | `1` | `lotRounded` округляет вниз, `0` → позиция не открывается |

> TODO (roadmap): вынести 20%/2%/4% в конфиг бэктеста (`bt.capital-slice`, `bt.sl-percent`, `bt.tp-percent`).
