...
class FuturesStopResolverTest {
    ...
    fun `atr maps to points via multiplier`() {
        ...
        assertEquals(40, resolver.resolve(BigDecimal("0.20"), priceStep, config.toFuturesAtrStopPolicy()))
    }
    ...
    fun `atr stop clamps to configured bounds`() {
        assertEquals(
            ...
            resolver.resolve(BigDecimal("4.0"), priceStep, config.toFuturesAtrStopPolicy()),
        )
        assertEquals(
            config.futuresAtrStopMinPoints,
            resolver.resolve(BigDecimal("0.03"), priceStep, config.toFuturesAtrStopPolicy()),
        )
    }

    @Test
    fun `null atr falls back to default stop points`() {
        assertEquals(config.defaultStopLossPoints, resolver.resolve(null, priceStep, config.toFuturesAtrStopPolicy()))
    }

    @Test
    fun `non positive price step falls back to default`() {
        assertEquals(config.defaultStopLossPoints, resolver.resolve(BigDecimal("0.20"), BigDecimal.ZERO, config.toFuturesAtrStopPolicy()))
    }
    ...
    fun `degenerate atr mapping falls back to default`() {
        ...
        assertEquals(
            zeroMultiplier.defaultStopLossPoints,
            resolver.resolve(BigDecimal("0.20"), priceStep, zeroMultiplier.toFuturesAtrStopPolicy()),
        )
    }
    ...
    fun `disabled flag returns default even with valid atr`() {
        ...
        assertEquals(
            disabled.defaultStopLossPoints,
            resolver.resolve(BigDecimal("0.20"), priceStep, disabled.toFuturesAtrStopPolicy()),
        )
        assertEquals(
            disabled.defaultStopLossPoints,
            resolver.resolve(null, priceStep, disabled.toFuturesAtrStopPolicy()),
        )
    }
}
