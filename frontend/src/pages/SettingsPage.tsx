...
export default function SettingsPage() {
  const {
    ...
    error,
    reload,
  } = useFetch<BotSettings>("/api/v1/settings");
  ...
  return (
    <div>
      ...
      >
        ...
        <label>
          ...
          <input
            ...
            onChange={(e) => update("riskEnabled", e.target.checked)}
            style={{ marginLeft: 8 }}
          />
        </label>
        <label>
          ...
          <input
            type="number"
            value={form.maxPositionRub}
            onChange={(e) => update("maxPositionRub", Number(e.target.value))}
            style={{ marginLeft: 8 }}
          />
        </label>
        <label>
          ...
          <input
            type="number"
            value={form.maxDailyLossRub}
            onChange={(e) => update("maxDailyLossRub", Number(e.target.value))}
            style={{ marginLeft: 8 }}
          />
        </label>
      ...
      <div style={{ marginTop: 20, color: "#666", fontSize: 13 }}>
        Trading mode is controlled via env <code>TRADING_MODE</code> (SIMULATION
        | LIVE).
      </div>
    </div>
  );
