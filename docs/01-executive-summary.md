...
- **Сохраняет дневной P&L в БД** — `daily_risk_snapshot`, восстановление после рестарта (раздел 6.6).
- **Emergency stop реализован** — `POST /api/v1/bot/emergency-stop` (раздел 5.8); авто-стоп по убытку — roadmap.
...
| Переобучение стратегии | бэктест с критериями приёма: Sharpe > 1.2, MDD < 15%, PF > 1.3, ≥ 200 сделок |
...
- Дневной лимит в 50 000 ₽ останавливает **все** входы на остаток дня (состояние в `daily_risk_snapshot`, восстановление после рестарта — раздел 5.6).
...
| Могу ли я остановить бота? | `POST /api/v1/bot/emergency-stop` — блокирует входы, опционально ликвидирует позиции; снятие — `POST /api/v1/bot/resume`. |
